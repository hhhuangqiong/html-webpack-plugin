...
const promisify = require('html-webpack-plugin-mp/node_modules/util.promisify');

const vm = require('vm');
const fs = require('fs');
const _ = require('html-webpack-plugin-mp/node_modules/lodash');
const path = require('path');
const childCompiler = require('html-webpack-plugin-mp/lib/compiler.js');
const prettyError = require('html-webpack-plugin-mp/lib/errors.js');
const chunkSorter = require('html-webpack-plugin-mp/lib/chunksorter.js');
...
class HtmlWebpackPlugin {
  ...
  apply (compiler) {
    ...
    const originPath = compiler.options.output.path;
    // compiler.options.output.path = '/Users/daisyhhuang/Documents/module/mmbizweb2/template/zh_CN/htmledition/pages';
    ...
    if (compiler.hooks) {
      compiler.hooks.compilation.tap('HtmlWebpackPluginHooks', compilation => {
        const SyncWaterfallHook = require('html-webpack-plugin-mp/node_modules/tapable').SyncWaterfallHook;
        const AsyncSeriesWaterfallHook = require('html-webpack-plugin-mp/node_modules/tapable').AsyncSeriesWaterfallHook;
    ...
    (compiler.hooks ? compiler.hooks.emit.tapAsync.bind(compiler.hooks.emit, 'HtmlWebpackPlugin') : compiler.plugin.bind(compiler, 'emit'))((compilation, callback) => {
      ...
      .then(() => compilationResult))
        ...
        .then(html => {
          ...
          this.html = html;
          const basicPath = compiler.options.output.path + '/';
          const filePath = basicPath + self.options.filename;

          let realPath = self.options.filename.split('/');
          let str = '';
          ...
          const folderPath = basicPath + str;
    ...
    return Promise.resolve()
      ...
      .then(html => {
        if (self.options.minify) {
          const minify = require('html-webpack-plugin-mp/node_modules/html-minifier').minify;
